...
import { sequentialNQLDetailsService } from '../../services/sequentialNQLDetailsService';

const initializeSequentialNQLDetailsTable = async (req: Request, res: Response) => {
  ...
  try {
    for (const row of rows.slice(1)) {
      const [topic, subTopic, questionType, grade, questionSetSequence] = row;
      ...
      const nqlTypeMapping = await subTopicNQLTypeMappingService.find(topic, subTopicIdentifier, questionType, NQLType.SEQUENTIAL);
      if (!nqlTypeMapping) {
      ...
      const questionSetSequences = questionSetSequence.replace('{', '').replace('}', '').trim().split(';');

      for (const data of questionSetSequences) {
        const [sequence, questionSetId] = data.split(':');
        await sequentialNQLDetailsService.create(
          {
            nql_type_mapping_id: nqlTypeMapping.identifier,
            class_id: classId,
            question_set_x_id: questionSetId.trim(),
            sequence: +sequence,
...
export default initializeSequentialNQLDetailsTable;

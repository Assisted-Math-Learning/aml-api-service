...
import { accuracyThresholdBasedNQLDetailsService } from '../../services/accuracyThresholdBasedNQLDetailsService';

const initializeAccuracyThresholdBasedNQLDetailsTable = async (req: Request, res: Response) => {
  ...
  try {
    for (const row of rows.slice(1)) {
      const [topic, subTopic, questionType, numberOfQuestionsToGive] = row;
      ...
      const nqlTypeMapping = await subTopicNQLTypeMappingService.find(topic, subTopicIdentifier, questionType, NQLType.ACCURACY_THRESHOLD_BASED);
      if (!nqlTypeMapping) {
        const code = 'NO_SEQUENTIAL_NQL_TYPE_MAPPING_FOUND';
        throw amlError(code, `No sequential NQL type mapping found:: topic: ${topic}, sub_topic: ${subTopic}, question_type: ${questionType}`, 'BAD_REQUEST', 400);
      }

      if (Number.isNaN(+numberOfQuestionsToGive)) {
        const code = 'INVALID_THRESHOLD';
        throw amlError(code, `Invalid numberOfQuestionsToGive value: ${numberOfQuestionsToGive}`, 'BAD_REQUEST', 400);
      }

      await accuracyThresholdBasedNQLDetailsService.create(
        {
          nql_type_mapping_id: nqlTypeMapping.identifier,
          num_of_questions_to_give: +numberOfQuestionsToGive,
...
export default initializeAccuracyThresholdBasedNQLDetailsTable;
